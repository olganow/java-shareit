...
class UserControllerTest {
    ...
    @Test
    public void createUserWithEmptyNameValidationTest() throws Exception {
        userDto.setName("");
        when(userService.createUser(any())).thenThrow(new ValidationException("Name is not valid, name is empty"));
        mockMvc.perform(post("/users")
                        .content(objectMapper.writeValueAsString(userDto))
                        .contentType("application/json")
                        .accept(MediaType.APPLICATION_JSON))
    ...
    public void createUserWithEmptyEmailValidationTest() throws Exception {
        userDto.setEmail("");
        when(userService.createUser(any())).thenThrow(new ValidationException("Email is not valid, email is empty"));
        mockMvc.perform(post("/users")
                        .content(objectMapper.writeValueAsString(userDto))
                        .contentType("application/json")
                        .accept(MediaType.APPLICATION_JSON))
    ...
    public void createUserWithWrongEmailTest() throws Exception {
        ...
        when(userService.createUser(any())).thenThrow(new ValidationException("Email isn't valid"));
        mockMvc.perform(post("/users")
                        .content(objectMapper.writeValueAsString(userDto))
                        .contentType("application/json")
                        .accept(MediaType.APPLICATION_JSON))
